...
import { NewsReader } from "@/components/news/options/NewsReader";
import { NewsSwitch } from "@/components/news/options/NewsSwitch";
import { newsArticles, newsCopy } from "@/data/news";

export const metadata: Metadata = {
  title: "News option F — Reader — SEVENPM",
  robots: { index: false },
};

/** News option F: index and story on one screen — headlines down the left, the chosen piece open on the right. */
export default function NewsReaderPreview() {
  return (
    <>
      <MotionProvider />
      <SiteHeader />
      <main>
        <section className="relative py-10 xl:py-16">
          <div className="shell flex flex-col gap-10">
            ...
            <NewsReader articles={newsArticles} />
          </div>
        </section>
