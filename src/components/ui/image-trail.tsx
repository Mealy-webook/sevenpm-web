 ...
 * From 21st.dev, kept as published apart from one addition: `hideCursor`
 * (default true) so a host that keeps its own cursor and needs native touch
 * scrolling can opt out of `cursor-none touch-none`.
 */
...
export type ImageTrailProps = {
  ...
  /** Hide the native cursor and block touch scrolling over the trail. */
  hideCursor?: boolean;
...
const DEFAULT_IMAGES: ImageTrailImage[] = [
  {
    src: "https://cdn.21st.dev/assets/mirror/7a/7a7dc1fcb058cacc02200a70503aa8f1234f030611befdf794dc1e97e43e40cd.jpg",
    alt: "Trail image 1",
  },
  {
    src: "https://cdn.21st.dev/assets/mirror/5d/5dfea55e544436a486b6e7aca3291e09cc3cf70029dbce7c270abb6b31910c6d.jpg",
    alt: "Trail image 2",
  },
  {
    src: "https://cdn.21st.dev/assets/mirror/26/26faaac3a7d375d5824e8da76fd594e0bdbad2ee4e88d94c6b1fb62a52b642a3.jpg",
    alt: "Trail image 3",
  },
];
...
export function ImageTrail({
  ...
  const resolvedDuration = Math.max(
    120,
    duration ?? trailSettings?.duration ?? 1000,
  );
  const resolvedSpacing = Math.max(4, spacing ?? trailSettings?.spacing ?? 40);
  const resolvedSmoothness = clamp(
    smoothness ?? trailSettings?.smoothness ?? 0.7,
    0,
    1,
  );
  const resolvedImageSize = Math.max(
    20,
    imageSize ?? appearance?.imageSize ?? 100,
  );
  const resolvedCornerRadius = Math.max(
    0,
    cornerRadius ?? appearance?.cornerRadius ?? 4,
  );
  ...
  const resolvedFadeInBlur = Math.max(
    0,
    fadeInBlur ?? animation?.fadeInBlur ?? 0,
  );
  const resolvedFadeOutBlur = Math.max(
    0,
    fadeOutBlur ?? animation?.fadeOutBlur ?? 5,
  );
  ...
  const addTrailImage = React.useCallback(
    (x: number, y: number) => {
      ...
      if (distance(nextPosition, lastPositionRef.current) < resolvedSpacing)
        return;
  ...
  const handleTouchMove = React.useCallback(
    (event: TouchEvent) => {
      ...
      if (!touch) return;
      if (hideCursor) event.preventDefault();
      updateFromPointer(touch.clientX, touch.clientY);
    },
    [hideCursor, updateFromPointer],
  );
  ...
  }, [handleMouseMove, handleTouchMove, hideCursor, staticPreview]);
  ...
  const calculateMagneticOffset = React.useCallback(
    ...
    [
      magneticEffect,
      mousePosition,
      resolvedMagneticRadius,
      resolvedMagneticStrength,
    ],
  );
  ...
  return (
    <div
      ...
      className={cn(
        ...
        !staticPreview && hideCursor && "cursor-none touch-none",
